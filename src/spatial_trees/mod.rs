...
mod kd_tree {
    ...
    fn split_vec<N : Dim, TItem : NPoint<N> + Copy + 'static>(pts : &Vec<TItem>, dim_count :usize) -> (Option<Vec<TItem>>, Option<TItem>, Option<Vec<TItem>>) where DefaultAllocator: Allocator<f64, N> {
        unsafe {
        ...
        match pts.first() {
            Some(firstPt) => {
                ...
                match split_vec(pts, dimIdx) {
                    ...
                    _                                   => panic!("Should be impossible to have a first point but still split into (None, None, None)"),
                }
            }
            None => {
                KdTreeImpl::Empty()
